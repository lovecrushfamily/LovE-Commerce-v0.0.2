...
create table user_CensorStaff ( StaffID int primary key,
							StaffName varchar(255) not null,
							PhoneNumber varchar(255) not null,
							Image_ varchar(255) not null,
							DateOfBirth date not null,
							Gender bit not null,
							foreign key (StaffID) references user_.Account (AccountID) on delete cascade on update cascade);
...
create table user_.Customer (	CustomerID int primary key,
						...
						foreign key (CustomerID) references user_.Account(AccountID) on delete cascade on update cascade);
...
create table production.Shop( ShopID int identity(1,1) primary key,
							...
							PhoneNumber varchar(255) not null,
							Image_ varchar(255) not null,
							OwnerID int references user_.Customer(CustomerID) on delete cascade on update cascade);
...
create table system_.Category ( CategoryID int identity(1,1) primary key,
								...
								AttributeList varchar(255) default 'no_needed' not null)
...
create table production.Product (ProductID int identity primary key,
								...
								CategoryID int references system_.Category(CategoryID) on delete cascade on update cascade,
								ShopID int references production.Shop(ShopID) on delete cascade on update cascade)
...
create table production.Order_ (OrderID int identity(1,1) primary key,
								CustomerID int references user_.Customer(CustomerID) on delete cascade on update cascade,
...
create table production.OrderDetail (OrderDetailID int identity(1,1) primary key ,
									OrderID int references production.Order_(OrderID) on delete cascade on update cascade,
...
create table system_.Notification_ (NotificationID int identity(1,1) primary key,
									ReceiverID int references user_.Customer(CustomerID) on delete cascade on update cascade,
...
create table system_.Comment (CommentID int identity(1,1) primary key,
								ProductID int references production.Product(ProductID) on delete cascade on update cascade,
								CustomerID int references user_.Customer(CustomerID),
								Star tinyint not null,
								Content varchar(255) not null,
								DateOfComment Date not null);

create table system_.ShopppingCart (CustomerID int references user_.Customer(CustomerID) on delete cascade on update cascade,
									ProductID int references production.Product(ProductID), -- create trigger delete shopping cart when product got deleted.
